...
def bootstrap_runtime_observer(service_name: str):
    """Initialize Runtime Observer for non-FastAPI processes (workers, lambdas).

    Returns the observer instance (or None on failure) so callers can flush
    ...
    try:
        ...
        os.environ.setdefault("RUNTIME_OBSERVER_SERVICE_NAME", service_name)
        observer = init_runtime_observer.from_env(service_name=service_name)
        logger.info("Runtime Observer: bootstrapped worker %s", service_name)
        return observer
    except Exception:
        logger.exception("Runtime Observer bootstrap failed")
        return None
