...
Tab outputs (tabs.json) are written into the same directory as the input
audio file — the song folder created by inference_demucs.
...
from tabs_generator.config import Settings
...
class StorageBackend(Protocol):
    ...
    def resolve_sibling(self, input_path: str, filename: str) -> str | None:
        """Resolve a sibling file in the same directory as input_path.

        Returns the storage path if the sibling exists, None otherwise.
        """
        ...
...
class LocalStorage:
    ...
    def _resolve_local_path(self, input_path: str) -> str:
        """Resolve an input path to an on-disk file.

        Callers may pass either:
        - an absolute path (e.g. /Users/.../local_bucket/.../guitar.mp3)
        - a storage key relative to base_path (e.g. tabs_generation/guitar.mp3)
        """
    ...
    def resolve_input(self, input_path: str) -> str:
        local_path = self._resolve_local_path(input_path)
        if not os.path.isfile(local_path):
            raise FileNotFoundError(
                f"Input file not found: {input_path} (resolved={local_path})"
            )
        return local_path

    def store_outputs(self, local_output_dir: str, input_path: str) -> str:
        """Copy output files into the parent directory of input_path."""
        dest = Path(self._resolve_local_path(input_path)).parent
    ...
    def file_exists(self, path: str) -> bool:
        return os.path.isfile(self._resolve_local_path(path))

    def resolve_sibling(self, input_path: str, filename: str) -> str | None:
        parent = Path(self._resolve_local_path(input_path)).parent
...
class S3Storage:
    ...
    def resolve_sibling(self, input_path: str, filename: str) -> str | None:
        parent_prefix = "/".join(input_path.split("/")[:-1])
        sibling_key = f"{parent_prefix}/{filename}" if parent_prefix else filename
        if self.file_exists(sibling_key):
            return sibling_key
        return None

    def read_json(self, path: str) -> Any:
        local_path = self.resolve_input(path)
        with open(local_path) as f:
            return json.load(f)
