...
Lyrics outputs (lyrics.json) are written into the same
...
from lyrics_generator.config import Settings
...
class LocalStorage:
    ...
    def _resolve_local_path(self, input_path: str) -> str:
        ...
        In local/test environments, callers may pass either:
        - an absolute path (e.g. /Users/.../local_bucket/.../vocals.mp3)
        - a storage key relative to base_path (e.g. bob_dylan/.../vocals.mp3)

        We normalize both into an absolute path on disk.
        """
    ...
    def store_outputs(self, local_output_dir: str, input_path: str) -> str:
        ...
        for filename in os.listdir(local_output_dir):
            ...
            if os.path.isfile(local_path):
                ...
                logger.info(
                    "Uploading %s -> s3://%s/%s", local_path, self._bucket, s3_key
                )
